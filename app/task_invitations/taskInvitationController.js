    ...
    .then(function(task_invitation) {
        ...
        task.save()
            .then(function (task) {
                ...
                task_invitation.populate([{ path: 'sender', select: '_id name email position avatar_url' }, { path: 'receiver', select: '_id name username email position avatar_url' }]).execPopulate()
                    .then(function (task_invitation) {
                        res.status(201).json({
                            success: true,
