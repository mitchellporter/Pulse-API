...
exports.getUpdates = function(req, res, next) {
	...
	function findUpdatesThatNeedResponses(callback) {
		...
		.populate([{ path: 'task', populate: [{ path: 'assigner', select: '_id name position email avatar_url' }, { path: 'assignees', select: '_id name username position email avatar_url' }] }, { path: 'responses.assignee', select: '_id name username position email avatar_url' } ]) // task.assigner
